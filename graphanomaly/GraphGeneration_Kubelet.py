...
root_path = r'/home/rcp_user/suzhang/graph/openstack/Logs2Graph'
# =============================================================================
# PreStep 1: Load parsered dataset OpenStack
# =============================================================================
import pandas as pd
from tqdm import tqdm
import pandas as pd
...
MyDataName = "OpenStack"

df = pd.read_csv(root_path + '/Data/OpenStack/Kubelet.log_structured.csv', sep=',')
df['GroupId'] = df['Podid']

raw_df = df[["LineId","PodID","GroupId", "EventTemplate"]] 
print(raw_df.head())
...
def GraphConstruction(my_example_df, graph_count_index, graph_loc_index, my_node_accum, new_node_accum):
    ...
    # 1. OpenStack_A.txt
    # 2. OpenStack_edge_attributes.txt
    # 3. OpenStack_graph_indicator.txt
    # 4. OpenStack_graph_labels.txt
    # 5. OpenStack_node_attributes.txt
    # =============================================================================
    ...
    fp_A = root_path + "/Data/OpenStack/Graph/TempRaw/" + MyDataName + "_A.txt"
    ...
    fp_edge_weight = root_path + "/Data/OpenStack/Graph/TempRaw/" + MyDataName + "_edge_attributes.txt"
    ...
    fp_graph_indicator = root_path + "/Data/OpenStack/Graph/TempRaw/" + MyDataName + "_graph_indicator.txt"
    ...
    df_label = pd.read_csv(root_path + "/Data/OpenStack/anomaly_label.csv", sep=',')
    ...
    fp_graph_labels = root_path + "/Data/OpenStack/Graph/TempRaw/" + MyDataName + "_graph_labels.txt"
    ...
    fp_node_attributes = root_path + "/Data/OpenStack/Graph/TempRaw/" + MyDataName + "_node_attributes.txt"
...
def ConcatGraphs(ReadGraph, graph_count_index, my_node_accum, new_node_accum, adj_type):
    ...
    # 1. Adj Matrix
    # =============================================================================
    fp_A = root_path + "/Data/OpenStack/Graph/Raw/" + MyDataName + "_A.txt"
    ...
    # =============================================================================
    fp_edge_weight = root_path + "/Data/OpenStack/Graph/Raw/" + MyDataName + "_edge_attributes.txt"
    ...
    df_graph_indicator = pd.DataFrame(columns=["indicator"])    
    df_graph_indicator["indicator"] = [graph_count_index+1]*len(new_node_accum)
    fp_graph_indicator = root_path + "/Data/OpenStack/Graph/Raw/" + MyDataName + "_graph_indicator.txt"
    ...
    fp_graph_labels = root_path + "/Data/OpenStack/Graph/Raw/" + MyDataName + "_graph_labels.txt"
    ...
    fp_node_attributes = root_path + "/Data/OpenStack/Graph/Raw/" + MyDataName + "_node_attributes.txt"
    ...
    if adj_type == 'ib':
        ...
        # =============================================================================
        fp_A2 = root_path + "/Data/OpenStack/Graph/Raw/" + MyDataName + "_A2.txt"
        ...
        # =============================================================================
        fp_edge_weight2 = root_path + "/Data/OpenStack/Graph/Raw/" + MyDataName + "_edge_attributes2.txt"
...
# ==== 你已有的日志文件 ====
raw_df = pd.read_csv(root_path + "/Data/Kubelet/kubelet_log_parsed.csv")

# ==== 你已有的标签文件 ====
all_event_df = pd.read_csv(root_path + "/Data/Kubelet/anomaly_label.csv", sep=",")
...
def draw_sample(num_samples, anomaly_per, draw_code):
    ...
    if draw_code == 1:
        ...
        sample_drawn = anomaly_index_list + normal_index_list

    list_group = [group_to_check[idx] for idx in sample_drawn]
    return list_group, sample_drawn


# ==== 清空输出目录 ====
import os, shutil
folder = root_path + "/Data/Kubelet/Graph/Raw"
...
# ==== 采样并构建图 ====
MyDataName = "Kubelet"   # 保证你拼接函数路径用这个
num_graphs_to_test = 100
anomaly_perentage = 0.2
draw_code_val = 1
...
for group_name in tqdm(list_group):
    ...
    # ==== Step 2. 读取图 ====
    MyReadGraph, empty_index = read_tu_data(folder=root_path + "/Data/Kubelet/Graph/TempRaw",
                                            prefix=MyDataName,
                                            adj_type='ib')
